...
**Expanded** (click popup; also the desktop face): no second ring. Session Allowance and Weekly Allowance are **horizontal bars** with `%` and time-to-reset under each label, same colour bands as the compact ring. Usage Credit is a status line, not a third bar — reading `Usage credit: {spent} of {ceiling}` in USD, falling back to the status word when the helper sends no amounts. Consumed Usage is a `Kirigami.FormLayout` (session / today / week / month; List Price then tokens). Account label at the top. Breeze spacing; no custom brand cards.
